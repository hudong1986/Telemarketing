...
public class ByteUtil {
	 ...
	 * @return
	 */
	public static int getUnShort(byte[] bytes) {
		return ((0xff & (int) bytes[0]) | (0xff00 & (bytes[1] << 8)));
	}
	...
	public static String getString(byte[] bytes, int index, int count, String charsetName) {
		return getString(subBytes(bytes, index, count), charsetName);
	}

	public static String getString(byte[] bytes, int index, int count) {
		return getString(subBytes(bytes, index, count));
	}

	/**
	 * 返回字节数组的16进制字符串表示，每个字节间用-分隔
	 * 
	 * @param src
	 * @return
	 */
